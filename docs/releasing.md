...
5. Verify npm scope ownership and confirm each package's GitHub Actions trusted publisher still names `Apex-Med/apex-docx-pdf`, `publish-next.yml`, the protected `npm` environment, and publish-only permission.
...
The publication workflow uses GitHub's OIDC token (`id-token: write`) and npm provenance. It must not store or pass a long-lived npm token. The protected `npm` GitHub environment requires maintainer approval and restricts deployment to `main`. All 12 npm packages are bound to `Apex-Med/apex-docx-pdf`, `publish-next.yml`, the `npm` environment, and publish-only permission. A first publication of a new name in the set (currently `@apexmed/forms`) may need a one-time authenticated bootstrap before that trusted-publisher mapping exists. The publish job strips the empty `_authToken` line that `actions/setup-node` writes when `registry-url` is set, then upgrades npm only for the publish step so Trusted Publishing OIDC can run without changing `npm pack --json` during the quality gate.
