  ...
  const inspectFile = (filePath) => {
    ...
    if (STYLE_EXTENSIONS.has(ext)) {
      ...
      const visualDeclarations =
        evidence.match(
          /\b(?:color|background(?:-color)?|border(?:-color)?|font-family)\s*:/gi
        )?.length ?? 0
      if (
    ...
    if (![".html", ".htm"].includes(ext) && evidence.length > 300) {
      const embeddedCustomProperties =
        evidence.match(/--[a-z0-9_-]+\s*:/gi)?.length ?? 0
...
const STOP_REVIEW_PROVIDERS = new Set([
  "claude-code",
  "codex",
  "agents",
  "grok",
  ...
  for (const root of roots) {
    for (const rel of manifests) {
      ...
      if (raw?.hooks && valueHasHookMarker(raw.hooks)) {
        return STOP_REVIEW_PROVIDERS.has(IMPECCABLE_PROVIDER_ID)
          ? "stop"
          : "per-edit"
      }
    }
...
function appendSurfaceBriefContext(parts, ctx) {
  if (ctx.hasSurfaceBrief && ctx.surfaceBrief) {
    parts.push(
      `# SURFACE BRIEF (${ctx.surfaceBriefPath})\n\n${ctx.surfaceBrief.trim()}`
    )
    return
  }
