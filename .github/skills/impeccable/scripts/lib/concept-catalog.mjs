...
export function validateConceptEntry(
  concept,
  { existingForms = new Map(), axes = null } = {}
) {
  const errors = []
  ...
  if (!normalized) {
    ...
    errors.push(
      `duplicate concept form: ${id} and ${existingForms.get(normalized)}`
    )
  }
  if (
    ...
    errors.push(
      `concept ${id} must name a form and inherited structure after a comma`
    )
  }
  if (
    ...
    errors.push(
      `concept ${id} needs specific lineage metadata of 12–200 characters`
    )
  }
  if (!CONCEPT_STRENGTHS.has(concept?.strength)) {
    errors.push(
      `concept ${id} needs a strength of ${[...CONCEPT_STRENGTHS].join(", ")}`
    )
  }
  if (
  ...
  if (
    ...
    errors.push(
      `concept ${id} needs system grammar with exactly five rules of 12–180 characters`
    )
  } else {
    ...
    if (
      concept.system.some(
        (rule, index) => !rule.startsWith(SYSTEM_PREFIXES[index])
      )
    ) {
      errors.push(
  ...
  if (
    ...
    errors.push(
      `concept ${id} needs a vivid creative spark of 80–320 characters`
    )
  }
  if (
  ...
  if (
    /\b(?:live digital system|shared participatory system) modeled on\b/i.test(
      concept?.form || ""
    )
  ) {
  ...
  if (
    /\b(?:in the style of|styled like|copy of)\b/i.test(concept?.form || "")
  ) {
...
export function validateConceptCatalog(
  ...
  if (
    ...
    errors.push(
      "catalog.qualityBar.principle must define the universal creative bar"
    )
  }
  if (
    !Array.isArray(catalog?.qualityBar?.rejectIf) ||
    catalog.qualityBar.rejectIf.length < 5
  ) {
    errors.push(
      "catalog.qualityBar.rejectIf must define at least five rejection gates"
    )
  }
  if (
    !Array.isArray(catalog?.qualityBar?.reviewAxes) ||
    catalog.qualityBar.reviewAxes.length < 8
  ) {
    errors.push(
      "catalog.qualityBar.reviewAxes must define at least eight review axes"
    )
  }
  ...
  for (const well of catalog?.wells || []) {
    ...
    if (
      typeof well.description !== "string" ||
      well.description.trim().length < 40
    ) {
      errors.push(
        `well ${well.id || "(unknown)"} needs a description of at least 40 characters`
      )
    }
  ...
  for (const family of catalog?.families || []) {
    ...
    for (const concept of family.concepts) {
      ...
      errors.push(
        ...validateConceptEntry(concept, { existingForms: normalizedForms })
      )
  ...
  if (minimumTotal !== undefined && concepts.length < minimumTotal) {
    errors.push(
      `expected at least ${minimumTotal} concepts, found ${concepts.length}`
    )
  }

  if (
    !Number.isInteger(reviewData?.schemaVersion) ||
    reviewData.schemaVersion < 2
  ) {
  ...
  for (const [id, review] of Object.entries(reviewData?.reviews || {})) {
    ...
    if (
      ...
      (typeof review.note !== "string" ||
        !review.note.trim() ||
        review.note.length > 500)
    ) {
      errors.push(
        `review ${id} note must be a non-empty string of 500 characters or fewer`
      )
    }
    ...
    if (
      review?.breadth !== undefined &&
      !CONCEPT_BREADTHS.has(review.breadth)
    ) {
      errors.push(
        `review ${id} breadth must be one of ${[...CONCEPT_BREADTHS].join(", ")}`
      )
    }
    ...
    if (review?.allowedModes !== undefined) {
      if (
        !Array.isArray(review.allowedModes) ||
        review.allowedModes.length === 0
      ) {
        errors.push(
        ...
        errors.push(
          `review ${id} allowedModes may only contain ${[...SEED_MODES].join(", ")}`
        )
      } else if (
        new Set(review.allowedModes).size !== review.allowedModes.length
      ) {
        errors.push(`review ${id} allowedModes must not repeat a mode`)
      } else if (review.allowedModes.length === SEED_MODES.size) {
        errors.push(
          `review ${id} allowedModes lists every mode; omit the field instead`
        )
      }
    }
  }

  const wellTierById = new Map(
    (catalog?.wells || []).map((well) => [well.id, well.tier])
  )
