...
export function validateCompositionEntry(
  ...
  if (!normalized) {
    ...
    errors.push(
      `duplicate composition form: ${id} and ${existingForms.get(normalized)}`
    )
  }
  if (
    typeof composition?.form !== "string" ||
    composition.form.trim().length < 40 ||
    composition.form.trim().length > 360 ||
    !composition.form.includes(",")
  ) {
    errors.push(
      `composition ${id} must name a staging and its structural mechanism after a comma`
    )
  }
  if (
    typeof composition?.lineage !== "string" ||
  ...
  if (!COMPOSITION_SURFACES.has(composition?.surface)) {
    errors.push(
      `composition ${id} needs a surface of ${[...COMPOSITION_SURFACES].join(", ")}`
    )
  }
  // Grain: how much of the product this composes. Optional, and absence means
  // eligible at any grain, so nothing needs backfilling.
  if (
    composition?.grain !== undefined &&
    composition.grain !== null &&
    !isGrain(composition.grain)
  ) {
    errors.push(
      `composition ${id} grain "${composition.grain}" must be one of ${COMPOSITION_GRAINS.join(", ")}`
    )
  }
  ...
  if (composition?.platforms !== undefined && composition.platforms !== null) {
    ...
    if (!Array.isArray(list) || list.length === 0) {
      errors.push(
        `composition ${id} platforms must be a non-empty array, or omitted to allow every platform`
      )
    } else if (list.some((entry) => !isPlatform(entry))) {
      errors.push(
        `composition ${id} platforms may only contain ${COMPOSITION_PLATFORMS.join(", ")}`
      )
      ...
      errors.push(
        `composition ${id} platforms lists every platform; omit the field instead`
      )
    }
  }
  if (
    !Array.isArray(composition?.tags) ||
  ...
  if (
    ...
    errors.push(
      `composition ${id} needs grammar with exactly four rules of 12–180 characters`
    )
  } else {
    ...
    if (
      composition.grammar.some(
        (rule, index) => !rule.startsWith(COMPOSITION_GRAMMAR_PREFIXES[index])
      )
    ) {
      errors.push(
        `composition ${id} grammar must use staging, sequence, controls, and adaptation prefixes in order`
      )
    }
  }
  if (
    typeof composition?.spark !== "string" ||
  ...
  if (
    typeof composition?.webLeverage !== "string" ||
...
export function validateCompositionCatalog(
  ...
  for (const family of catalog?.families || []) {
    ...
    if (
      ...
      family.description.trim().length < 40
    ) {
      errors.push(
        `composition family ${family.id || "(unknown)"} needs a description`
      )
    }
  }
  for (const composition of catalog?.compositions || []) {
    ...
    if (!familyIds.has(composition.familyId)) {
      errors.push(
        `composition ${composition.id} must belong to a declared family, got: ${String(composition.familyId)}`
      )
    }
    errors.push(
  ...
  if (
    ...
    (catalog?.compositions || []).length < minimumTotal
  ) {
    errors.push(
      `expected at least ${minimumTotal} compositions, found ${(catalog?.compositions || []).length}`
    )
  }
  for (const [id, review] of Object.entries(reviewData?.reviews || {})) {
    ...
    if (typeof review?.formHash !== "string" || !review.formHash.trim()) {
      ...
      if (entry && review.formHash !== compositionContentHash(entry)) {
        errors.push(
          `composition review ${id} is stale: content changed since review`
        )
      }
    }
    ...
    if (
      ...
      errors.push(
        `composition review ${id} note must be a non-empty string of 500 characters or fewer`
      )
    }
  }
  return {
