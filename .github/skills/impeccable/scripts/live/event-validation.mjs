...
function validateAnnotationFields(msg) {
  if (
    ...
    typeof msg.screenshotPath !== "string"
  ) {
...
function validateInsertGenerate(msg) {
  ...
  if (
    !Number.isFinite(msg.placeholder.width) ||
    !Number.isFinite(msg.placeholder.height)
  ) {
...
function validateMountAck(msg) {
  ...
  if (!isValidMountVariant(msg.variant))
    return "variant_mounted: variant must be an integer 1-999"
  if (msg.url !== undefined) {
    if (typeof msg.url !== "string")
      return "variant_mounted: url must be string"
    if (msg.url.length > MOUNT_URL_MAX_LENGTH)
      return "variant_mounted: url too long"
  }
  return null
}
...
function validateMountFailure(msg) {
  ...
  if (!isValidMountVariant(msg.variant))
    return "variant_mount_failed: variant must be an integer 1-999"
  if (typeof msg.url !== "string" || !msg.url.trim())
    return "variant_mount_failed: url required"
  if (msg.url.length > MOUNT_URL_MAX_LENGTH)
    return "variant_mount_failed: url too long"
  if (typeof msg.error !== "string" || !msg.error.trim())
    return "variant_mount_failed: error required"
  if (msg.error.length > MOUNT_ERROR_MAX_LENGTH)
    return "variant_mount_failed: error too long"
  return null
}
  ...
  switch (msg.type) {
    ...
    case "agent_phase":
      ...
      if (typeof msg.phase !== "string" || !msg.phase)
        return "agent_phase: missing phase"
      ...
      if (
        ...
        (!Number.isFinite(msg.durationMs) || msg.durationMs < 0)
      ) {
    ...
    case "carbonize_cleanup":
      if (!isValidId(msg.id))
        return "carbonize_cleanup: missing or malformed id"
      if (!isValidId(msg.sessionId))
        return "carbonize_cleanup: missing or malformed sessionId"
      if (!msg.file || typeof msg.file !== "string")
        return "carbonize_cleanup: missing file"
      if (!isValidVariantId(String(msg.variantId)))
        return "carbonize_cleanup: missing or malformed variantId"
      return null
    default:
