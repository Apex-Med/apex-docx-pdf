...
export function bakeParamValues(css, params = [], values = {}) {
  ...
  for (const param of params || []) {
    ...
    chosen.set(param.id, {
      kind: param.kind,
      value: has ? values[param.id] : param.default,
    })
  }
  ...
  const bakeBody = (body) => {
    ...
    for (const [id, { kind, value }] of chosen) {
      const literal =
        kind === "toggle" ? normalizeToggleForVar(value) : String(value)
      ...
      (warnings || [])
        ...
        .map((w) =>
          String(componentSource)
            .slice(w.start.character, w.end.character)
            .trim()
        )
    )
  } catch {
...
export function pruneUnusedSelectors(
  componentSource,
  compileFn,
  { skipSelectors } = {}
) {
  ...
  const removed = []
  const skip =
    skipSelectors instanceof Set ? skipSelectors : new Set(skipSelectors || [])
  for (let pass = 0; pass < 3; pass++) {
    ...
    const unused = (warnings || [])
      ...
      .filter(
        (w) =>
          !skip.has(source.slice(w.start.character, w.end.character).trim())
      )
...
function removeSelectorAt(source, start, end) {
  ...
  if (kept.length === 0) {
    ...
    while (
      ruleStart > 0 &&
      (source[ruleStart - 1] === " " || source[ruleStart - 1] === "\t")
    )
      ruleStart--
    return {
      changed: true,
