...
function isAppRoot(dir) {
  ...
  return (
    hasDevConfig(dir) ||
    exists(path.join(dir, ".impeccable", "live", "config.json"))
  )
}
...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  const legacyRoot = resolveProjectRoot(
    absCwd,
    absTarget ? { targetPath: absTarget } : {}
  )
  const markerBound =
    absTarget &&
    insideOrEqual(targetDir, legacyRoot) &&
    insideOrEqual(legacyRoot, upperBound)
      ? legacyRoot
      : upperBound

  let appRoot = walkUp(targetDir, markerBound, (dir) =>
    isAppRoot(dir) ? dir : null
  )
  ...
  const effectiveRepoRoot =
    repoRoot && insideOrEqual(appRoot, repoRoot) ? repoRoot : appRoot
...
function hasLiveServer(appRoot) {
  ...
  try {
    const info = JSON.parse(
      fs.readFileSync(
        path.join(appRoot, ".impeccable", "live", "server.json"),
        "utf-8"
      )
    )
 ...
 * conflicting truth.
 */
export function resolveLiveRoots(
  cwd = process.cwd(),
  { targetPath = null } = {}
) {
  const absCwd = path.resolve(cwd)
  ...
  if (!targetPath) {
    ...
    if (gitRoot) {
      ...
      if (candidates.length > 0) {
        const liveApps = candidates.filter((manifest) =>
          hasLiveServer(manifest.appRoot)
        )
        const recoveringApps =
          liveApps.length > 0
            ? liveApps
            : candidates.filter((manifest) =>
                hasActiveDurableSession(manifest.appRoot)
              )
