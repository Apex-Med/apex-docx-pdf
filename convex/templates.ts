...
function validateMetadata(args: {
  ...
  assertJsonMetadata("Manifest", args.manifestJson, 256 * 1024)
  assertJsonMetadata("JSON schema", args.jsonSchemaJson, 256 * 1024)
  assertJsonMetadata("Starter data", args.starterDataJson, 256 * 1024)
...
export const findBySourceHash = query({
  ...
  handler: async (ctx, args) => {
    ...
    const template = await ctx.db
      ...
      .order("desc")
      .first()
    return template?.status === "deleting" ? null : template
  },
})
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    const renders = await ctx.db
      ...
      .withIndex("by_sessionId_and_templateId_and_createdAt", (q) =>
        q.eq("sessionId", args.sessionId).eq("templateId", args.templateId)
      )
      .take(25)
