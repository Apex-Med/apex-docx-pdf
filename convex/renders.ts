...
export const findCached = query({
  ...
  handler: async (ctx, args) => {
    ...
    const candidates = await ctx.db
      ...
      .order("desc")
      .take(8)
    return candidates.find((candidate) => candidate.pdfStorageId) ?? null
  },
})
...
export const begin = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (template.sourceHash !== args.templateHash) {
      throw new Error("Render template hash does not match its template")
    }
