...
Date: 2026-08-06. Scope: current source entrypoints, published `0.1.0-next.1` artifacts, and package validation. This is an API inventory, not a stable semver promise.
...
4. All 11 packages are published at `0.1.0-next.1`. A clean registry-only install imported the facade at engine version `0.0.0-phase.8`, found the complete installed AI surface, and successfully loaded the shipped inspector; npm audited all 36 installed dependency signatures successfully. Every package has a publish-only GitHub OIDC trusted publisher, but the approved workflow was cancelled with zero steps started during a major Actions outage and the local fallback has no first-party provenance attestation. That remains to be proved on a future automated prerelease.
