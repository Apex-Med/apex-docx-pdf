...
const nodes: Record<string, NodeSpec> = {
  ...
  template_tag: {
    ...
    parseDOM: [{ tag: "span[data-template-tag]" }],
    toDOM: (node) => [
      "span",
      {
        "data-template-tag": String(node.attrs.slug),
        "data-tag-id": String(node.attrs.tagId ?? ""),
        "data-node-id": node.attrs.nodeId ?? "",
        class: "apex-template-tag apex-template-tag--empty",
        style: [
          `font-family:${String(node.attrs.fontFamily)}`,
          `font-size:${Number(node.attrs.fontSize) / 20}pt`,
          `font-weight:${Number(node.attrs.fontWeight)}`,
          `font-style:${String(node.attrs.fontStyle)}`,
          `color:${String(node.attrs.color)}`,
        ].join(";"),
      },
      [
        "span",
        { class: "apex-template-tag__chip", contenteditable: "false" },
