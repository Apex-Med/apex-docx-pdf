...
4. Use `ai/skills/bind-apex-form/SKILL.md` when mapping `@apexmed/forms` questions onto template tags or converting answers into render data.
...
Prefer the unscoped `apex-docx-pdf` facade for application code. Use `@apexmed/browser` only for the Web Worker boundary, `@apexmed/forms` for the headless questionnaire model, and treat the lower-level `@apexmed/*` pipeline packages as advanced prerelease surfaces.
